...
```{r tab1_disp, results="asis", echo = F}
...
```{r tab2_disp, results="asis", echo=FALSE}
...
##T Table 15: Annual Trends in the Total Number of Recorded Vital Sign Records, the Mean Number of Encounters, the Mean Number of Patients and the Percent Change from the Previous Year, by Calendar Year
...
tab15_avg_encts <- odbc::dbGetQuery(con, 
  "select year(measure_date) as year,
    count(*) as nrows
	  from vital_signs
...
tab15_avg_pats <- odbc::dbGetQuery(con, 
  "select year(measure_date) as year,
    count(*) as nrows
	  from vital_signs
...
tab15_add <- tab15 %>% 
  bind_cols(., tab15_avg_encts, tab15_avg_pats) %>% 
  mutate(
    ptc_rec = if_else(is.na(nrows / dplyr::lag(nrows)), "--", paste0(round(nrows / dplyr::lag(nrows) * 100, 0), "%"))
  ) %>% 
  select(year, nrows, ptc_rec, nencts, npats, enct_val, pats_val)

tab15_add$year <- as.character(tab15_add$year)

```

```{r tab15_dis, results="asis"}

kable(tab8_add, col.names = c("Year", "Total Number of Recorded Vitals", "Percent Change from Previous Year", "Total Number of Encounters with Vital_Signs", "Total Number of Patients with Vitals", "Median Number of Vitals per Encounter (Min, Max)", "Median Number of Vitals per Patient (Min, Max)"), format.args = list(big.mark = ","))
