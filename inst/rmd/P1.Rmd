...
# mgk: load lib for GBQ R interface
# mgk: see https://github.com/r-dbi-bigrquery
...
# mgk: create connection to UCHealth CHORDS VDW by specifying project information
# mgk: UCHealth VDW project: hdcchvdwmedd
project <- "hdcchvdwmedd"
...
# mgk
sql <- "select count(*) as nrows, count(distinct person_id) as npats from chvdw.DEMOGRAPHICS"
tab1_dem <- query_exec(sql, project = project, use_legacy_sql = FALSE)
sql <- ""
...
# mgk
sql <- "select count(*) as nrows, count(distinct person_id) as npats, count(distinct enc_id) as nencts,
        'ADATE' as fieldname, min(ADATE) as mindt, max(ADATE) as maxdt from chvdw.ENCOUNTERS"
tab1_enc <- query_exec(sql, project = project, use_legacy_sql = FALSE)
sql <- ""
...
# mgk
# mgk -- Table is called CENSUS_LOCATION_ADDRESS_FILE in UCHealth. Investigate the difference
sql <- "select  count(*) as nrows, count(distinct person_id) as npats from chvdw.CENSUS_LOCATION_ADDRESS_FILE"
tab1_loc <- query_exec(sql, project = project, use_legacy_sql = FALSE)
sql <- ""
...
# mgk
sql <- "select count(*) as nrows, count(distinct person_id) as npats, 'ADATE' as fieldname,
    count(distinct enc_id) as nencts, min(ADATE) as mindt, max(ADATE) as maxdt from chvdw.DIAGNOSES"
tab1_dx <- query_exec(sql, project = project, use_legacy_sql = FALSE)
sql <- ""
...
sql <- "SELECT SUM(CASE WHEN BIRTH_DATE IS NULL THEN 1 ELSE 0 END) as BIRTH_DATE,
   SUM(CASE WHEN GENDER IS NULL THEN 1 ELSE 0 END) as GENDER,
   SUM(CASE WHEN PRIMARY_LANGUAGE IS NULL THEN 1 ELSE 0 END) as PRIMARY_LANGUAGE,
      SUM(CASE WHEN NEEDS_INTERPRETER IS NULL THEN 1 ELSE 0 END) as NEEDS_INTERPRETER,
      SUM(CASE WHEN RACE1 IS NULL THEN 1 ELSE 0 END) as RACE1,
         ...
         SUM(CASE WHEN SEXUAL_ORIENTATION IS NULL THEN 1 ELSE 0 END) as SEXUAL_ORIENTATION,
	    SUM(CASE WHEN GENDER_IDENTITY IS NULL THEN 1 ELSE 0 END) as GENDER_IDENTITY
	       FROM chvdw.DEMOGRAPHICS"
tab2_demo_null <- query_exec(sql, project = project, use_legacy_sql = FALSE)
sql <- ""
...
tab2_enc_null <- odbc::dbGetQuery(con, 
  "SELECT SUM(CASE WHEN ADATE IS NULL THEN 1 ELSE 0 END) as ADATE,
   ...
   SUM(CASE WHEN ENCTYPE IS NULL THEN 1 ELSE 0 END) as ENCOUNTER_SUBTYPE
      FROM ENCOUNTERS")
sql <- "SELECT SUM(CASE WHEN ADATE IS NULL THEN 1 ELSE 0 END) as ADATE,
   SUM(CASE WHEN DDATE IS NULL THEN 1 ELSE 0 END) as DDATE,
   SUM(CASE WHEN PROVIDER IS NULL THEN 1 ELSE 0 END) as PROVIDER,
      SUM(CASE WHEN ENCTYPE IS NULL THEN 1 ELSE 0 END) as ENCOUNTER_SUBTYPE
         FROM chvdw.ENCOUNTERS"
tab2_enc_null <- query_exec(sql, project = project, use_legacy_sql = FALSE)
sql <- ""
...
# mgk
# FAKE GEOCODING (absent)
# foobar

sql <- "SELECT SUM(CASE WHEN ADDRESS_1 IS NULL THEN 1 ELSE 0 END) as GEOCODE,
   SUM(CASE WHEN CITY IS NULL THEN 1 ELSE 0 END) as CITY_GEOCODE
      FROM chvdw.CENSUS_LOCATION_ADDRESS_FILE"
tab2_c1_null <- query.exec(sql, project = project, use_legacy_sql = FALSE)
sql <- ""
...
# mgk
sql <- "SELECT SUM(CASE WHEN ENC_ID IS NULL THEN 1 ELSE 0 END) as ENC_ID,
   SUM(CASE WHEN ADATE IS NULL THEN 1 ELSE 0 END) as ADATE,
   SUM(CASE WHEN DX IS NULL THEN 1 ELSE 0 END) as DX,
      SUM(CASE WHEN PRINCIPAL_DX IS NULL THEN 1 ELSE 0 END) as PRINCIPAL_DX,
      SUM(CASE WHEN PRIMARY_DX IS NULL THEN 1 ELSE 0 END) as PRIMARY_DX
            FROM chvdw.DIAGNOSES"
tab2_dx_null <- query.exec(sql, project = project, use_legacy_sql = FALSE)
sql <- ""
...
# mgk Vital Signs table not correct
sql <- "SELECT SUM(CASE WHEN ENC_ID IS NULL THEN 1 ELSE 0 END) as ENC_ID,
   SUM(CASE WHEN ENCTYPE IS NULL THEN 1 ELSE 0 END) as ENCTYPE,
   SUM(CASE WHEN HT IS NULL THEN 1 ELSE 0 END) as HT,
   SUM(CASE WHEN WT IS NULL THEN 1 ELSE 0 END) as WT,
   ...
   SUM(CASE WHEN TEMP_RAW IS NULL THEN 1 ELSE 0 END) as TEMP_RAW,
   SUM(CASE WHEN PULSE_RAW IS NULL THEN 1 ELSE 0 END) as PULSE_RAW
      FROM chvdw.VITAL_SIGNS"


 # tab2_vs_null2 <- query_exec(sql, project = project, use_legacy_sql = FALSE)
 sql <- ""
...
sql <- "SELECT SUM(CASE WHEN GENDER = 'U' THEN 1 ELSE 0 END) as GENDER,
   SUM(CASE WHEN PRIMARY_LANGUAGE = 'UNK' THEN 1 ELSE 0 END) as PRIMARY_LANGUAGE,
   SUM(CASE WHEN NEEDS_INTERPRETER = 'U' THEN 1 ELSE 0 END) as NEEDS_INTERPRETER,
   SUM(CASE WHEN RACE1 = 'UN' THEN 1 ELSE 0 END) as RACE1,
   SUM(CASE WHEN HISPANIC = 'U' THEN 1 ELSE 0 END) as HISPANIC,
   ...
   SUM(CASE WHEN GENDER_IDENTITY = 'UN' THEN 1 ELSE 0 END) as GENDER_IDENTITY
       FROM chvdw.DEMOGRAPHICS"
tab2_demo_uk2 <- query_exec(sql, project = project, use_legacy_sql = FALSE)
sql <- ""
...
sql <- "SELECT SUM(CASE WHEN PROVIDER = 'UNKNOWN' THEN 1 ELSE 0 END) as PROVIDER
      FROM chvdw.ENCOUNTERS"
tab2_enc_uk <- query_exec(sql, project = project, use_legacy_sql = FALSE)
sql <- ""
...
# mgk
sql <- "select  count(*) as nDemogRows FROM  chvdw.DEMOGRAPHICS"
nDemogRows <- query_exec(sql, project = project, use_legacy_sql = FALSE)
sql <- ""
...
# 		                                          ' person_id',
# 		                                         ', case when person_id in (select distinct person_id from', 'chvdw.ENCOUNTERS',' ) then 1 else 0 end as nPatsWithEncs ',
# 		                                        ',  case when person_id in (select distinct person_id from', 'chvdw.CENSUS_LOCATION_ADDRESS_FILE',' ) then 1 else 0 end as nPatsWithLoc ',
# 		                                        ',  case when person_id in (select distinct person_id from', 'chvdw.DIAGNOSES' ,' ) then 1 else 0 end as nPatsWithDiag ',
# 		                                        ',  case when person_id in (select distinct person_id from', 'chvdw.VITAL_SIGNS',' ) then 1 else 0 end as nPatsWithVital ',
# 		                                        '  FROM',  'chvdw.DEMOGRAPHICS',') as q',
#                         sep=" "
#                  ),
#                  max=maxQryRows
#                   )
...
sql <- "select  count(*) as nDemogRows, 
        count(distinct person_id) as nPatsDemog, 
        sum(nPatsWithEncs) as nPatsWithEncs, 
        sum(nPatsWithLoc) as nPatsWithLoc, 
        sum(nPatsWithDiag) as nPatsWithDiag, 
...
FROM (select distinct person_id , 
      ...
      case when person_id in (select distinct person_id from chvdw.VITAL_SIGNS) then 1 else 0 end as nPatsWithVital
      FROM chvdw.DEMOGRAPHICS ) as q"
demogSummary <- query_exec(sql, project = project, use_legacy_sql = FALSE)
sql <- ""
...
sql <-"SELECT gender, primary_language, race1, count(*) as nRows
FROM  chvdw.DEMOGRAPHICS
group by gender, primary_language, race1"
demgCounts <- query_exec(sql, project = project, use_legacy_sql = FALSE )
sql <-NULL
