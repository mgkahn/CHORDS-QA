...
Once you have download and installed R, RStudio.  You can download the `chordsTables` package from ([here](https://twitter.com/hadleywickham/status/504368538874703872)). You can download the package by running the following code in RStudio:
...
After you have selected the server and database names, RStudio will generate the requested report. This could take 20-50 minutes depending on the computer you are using and the amount of data that is being returned by the report.  The report is stored in the following directory `r paste0("C:/Users/", Sys.info()["login"], "/Documents")` (the My Documents folder for the current user), and the file name is the name of the argument passed to `run_repor()` (e.g., `P1.docx`).
